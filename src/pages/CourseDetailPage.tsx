...
const CourseDetailPage = () => {
  ...
  return (
    <div className="w-full">
      <NaverMap
        latitude={data?.data?.items.places[0].latitude}
        longitude={data?.data?.items.places[0].longitude}
        height={198}
      />
      <div className="h-full w-full rounded-t-[8px] bg-white px-[20px] pt-[12px] shadow-[2px_2px_12px_0px_rgba(0,0,0,0.2)]">
        <div className="flex items-center gap-[4px]">
          {data?.data?.items.line?.map(({ line, uuid }: { line: string; uuid: string }) => (
            <div
        ...
        <div className="mt-[12px] h-full">
          <div className="text-16">
            <span className="font-bold">{data?.data?.items.course_name.split(',')[0]}</span>
            <span className="font-medium">,{data?.data?.items.course_name.split(',')[1]}</span>
          </div>
        </div>
        <div>작업중...</div>
      </div>
